"use client";
import { editGbvInformationSchema } from "@/schema/gbvInformation";
import {
...
function EditForm({ gbvInformation }: Props) {
  ...
  const {
    ...
    resolver: zodResolver(editGbvInformationSchema),
    defaultValues: gbvInformation || {},
  });
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    try {
      const response = await fetch(
        `/api/gbvInformation/${gbvInformation?.id}`,
        {
          method: "PUT",
      ...
      if (response.status === 200) {
        toast.success("Gbv Information edited Successful", {
          position: "top-right",
        });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      {/* If yes, identify ALL services provided/ Was survivor referred to another service */}
      <div className="flex justify-between items-center gap-4">
        <div className="flex flex-col mt-2 w-full">
          <label className="text-xs font-semibold">
