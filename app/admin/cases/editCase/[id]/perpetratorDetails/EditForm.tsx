"use client";
import { editPerpetratorDetailsSchema } from "@/schema/perpetratorDetails";
import {
...
type Props = {
  perpetratorDetails: any;
};

function EditForm({ perpetratorDetails }: Props) {
  ...
  const {
    ...
    resolver: zodResolver(editPerpetratorDetailsSchema),
    defaultValues: perpetratorDetails || {},
  });
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    try {
      const response = await fetch(
        `/api/perpetratorDetails/${perpetratorDetails?.id}`,
        {
          method: "PUT",
      ...
      if (response.status === 200) {
        toast.success("Perpetrator details edited Successful", {
          position: "top-right",
        });
