...
import { getPerpetratorDetails } from "@/services/perpetratorDetails";
...
async function Page({ params }: PageProps) {
  const perpetratorDetails = await getPerpetratorDetails(params.id);
      ...
      <div className="bg-white rounded-lg p-4 w-full">
        <div className="bg-purple-50 p-2 rounded-t-md">
          <h1 className="text-md font-semibold">
            CASE ID - {perpetratorDetails?.case?.caseId}
          </h1>
        </div>
        <EditForm perpetratorDetails={perpetratorDetails} />
      </div>
    </div>
  );
}
